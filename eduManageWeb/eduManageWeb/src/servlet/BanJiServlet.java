...
public class BanJiServlet extends HttpServlet {
	...
	private void search(HttpServletRequest request, HttpServletResponse response) {
		...
		int max=bjDao.searchCount(bj);
		int yeNum=2;
		int yeMa=5;
		Pagination pagination=new Pagination(ye, max,yeNum,yeMa);
		ye=pagination.getYe();
		begin=(ye-1)*yeNum;
		List<BanJi>list =bjDao.searchByCondition(bj,begin,yeNum);
		try {
			request.setAttribute("bjs", list);
			request.setAttribute("p",pagination);
			request.setAttribute("bj",bj);
			request.getRequestDispatcher("WEB-INF/banji/list.jsp").forward(request, response);
		} catch (ServletException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (IOException e) {
	...
	private void showModify(HttpServletRequest request,
		...
		try {
			...
			request.setAttribute("bj",bj);
			request.getRequestDispatcher("WEB-INF/banji/modify.jsp").forward(request, response);
	...
	private void showAdd(HttpServletRequest request ,HttpServletResponse response){
		try {
			request.getRequestDispatcher("WEB-INF/banji/add.jsp").forward(request, response);
		} catch (ServletException | IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
